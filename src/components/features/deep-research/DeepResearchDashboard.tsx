...
import { Search, Send, Loader2, Download, BookOpen, ArrowLeft, StopCircle, Share2, PanelRightClose, PanelRightOpen, Globe, FileText, Bot, RotateCcw } from "lucide-react";
...
interface Message {
  role: "user" | "assistant";
  content: string;
  type?: "text" | "research_progress" | "research_report" | "cancellation";
...
export const DeepResearchDashboard = ({ initialQuery, conversationId, initialMessages = [], initialSources, onBack, conversationCreatedAt }: DeepResearchDashboardProps) => {
  ...
  const handleStop = () => {
    if (abortControllerRef.current) {
      ...
      setMessages(prev => [...prev, { role: "assistant", content: "Research stopped by user.", type: "cancellation" }]);
    }
  };
  ...
  useEffect(() => {
    ...
    if (initialQuery && !hasAutoStarted.current && messages.length === 0) {
      ...
      // Safety check: Don't auto-start for old conversations that happen to differ empty history
      // (e.g. if previous persistence failed, we don't want to surprise the user by restarting)
      const isRecent = conversationCreatedAt
        ? (new Date().getTime() - new Date(conversationCreatedAt).getTime()) < 60000 // 1 minute
        : true; // Default to true if not provided (new chats)
  ...
  }, [initialQuery, initialSources, messages.length, conversationCreatedAt]);
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#1a1a1c] text-gray-100 relative">
        ...
        {messages.map((msg, idx) => (
          ...
          >
            <div className={`${msg.role === 'user'
              ? 'max-w-3xl ml-auto w-fit'
              : 'w-full max-w-none' // Assistant messages take full width
              }`}>
              ...
              {msg.role === 'user' ? (
                ...
                msg.type === 'research_report' ? (
                  ...
                  <div className="flex flex-col items-start gap-2 text-orange-400/80 text-sm pl-1">
                    <p>{msg.content}</p>
                    <button
                      onClick={() => {
                        // Retry the user's last message
                        const lastUserMsg = [...messages].reverse().find(m => m.role === 'user');
                        if (lastUserMsg) {
                          startResearchStream(lastUserMsg.content);
                        }
                      }}
                  ...
                  <div className="bg-[#2a2a2c] border border-white/5 rounded-2xl rounded-tl-sm px-6 py-4 text-gray-200 shadow-sm leading-relaxed whitespace-pre-wrap w-full">
                    {msg.content}
                  </div>
                )
              )}
      ...
      <div className="p-4 bg-[#1a1a1c] border-t border-white/5 z-20">
        <div className="max-w-3xl mx-auto w-full">
          <div className={`relative bg-[#2a2a2c] transition-all duration-300 ring-1 ring-white/5 focus-within:ring-teal-500/50 rounded-2xl shadow-xl`}>
            {/* Main Input */}
            <div className="px-5 py-4">
              <textarea
                ...
                rows={1}
                className="w-full bg-transparent border-none outline-none text-gray-200 placeholder:text-gray-500 text-base resize-none max-h-[200px] overflow-y-auto"
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-700/50">
              ...
              <div className="flex items-center">
                {isResearching ? (
                  <button
                    onClick={handleStop}
                    className="p-2.5 bg-red-500/10 text-red-400 border border-red-500/50 rounded-xl hover:bg-red-500/20 transition-all shadow-lg"
