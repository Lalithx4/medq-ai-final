...
export function CreditsDisplay({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) {
    ...
    return (
      <Link href="/settings?tab=credits" className="block">
        <div
          className={`flex items-center justify-center w-10 h-10 rounded-lg cursor-pointer transition-all hover:scale-105 relative ${hasNoCredits
  ...
  return (
    <div className="space-y-2">
      <Link href="/settings?tab=credits" className="block">
        <div
          className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all hover:scale-105 ${hasNoCredits
            ? "bg-destructive/10 text-destructive border border-destructive/20"
            : isLowCredits
              ? "bg-yellow-500/10 text-yellow-600 border border-yellow-500/20"
              : "bg-primary/10 text-primary border border-primary/20"
        ...
        >
          ...
          <div className="flex-1">
            ...
            {hasNoCredits && (
              <div className="text-xs opacity-75">Out of credits</div>
            )}
          </div>
          {(isLowCredits || hasNoCredits) && (
