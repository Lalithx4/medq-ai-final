      ...
      const generateImage = async (prompt: string) => {
        ...
        try {
          ...
          if (imageSource === "stock") {
            // Use stock images (Unsplash or OpenI)
            if (stockImageProvider === "openi") {
              const openiResult = await getImageFromOpenI(prompt);
              if (openiResult.success && openiResult.url) {
                result = {
                  success: true,
                  image: { url: openiResult.url },
                };
              }
            } else {
              ...
              <div ref={imageRef}>
                ...
                >
                  ...
                  {isGenerating ? (
                    ...
                    <div className="presentation-image bg-muted flex items-center justify-center text-muted-foreground text-sm min-h-[200px]">
                      {props.element.query ? `Loading: ${props.element.query}` : "No image"}
                    </div>
                  )}
                  <ResizeHandle
