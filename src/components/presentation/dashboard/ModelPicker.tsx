...
export function ModelPicker({
  ...
  useEffect(() => {
    if (!hasRestoredFromStorage.current) {
      ...
      if (savedModel) {
        debug.log("Restoring model from localStorage:", savedModel);
        setModelProvider(
          savedModel.modelProvider as "openai" | "ollama" | "lmstudio" | "cerebras",
        );
  ...
  const getCurrentModelOption = () => {
    ...
    if (currentValue.startsWith("cerebras-")) {
      return {
        label: modelId || "Cerebras",
        icon: Bot,
      };
    }
  ...
  const handleModelChange = (value: string) => {
    debug.log("Model changed to:", value);
    if (value === "openai") {
      setModelProvider("openai");
      setModelId("");
      setSelectedModel("openai", "");
      debug.log("Saved to localStorage: openai, ''");
    } else if (value.startsWith("ollama-")) {
      const model = value.replace("ollama-", "");
      setModelProvider("ollama");
      setModelId(model);
      setSelectedModel("ollama", model);
      debug.log("Saved to localStorage: ollama,", model);
      ...
      debug.log("Saved to localStorage: lmstudio,", model);
    } else if (value.startsWith("cerebras-")) {
      const model = value.replace("cerebras-", "");
      setModelProvider("cerebras");
      setModelId(model);
      setSelectedModel("cerebras", model);
      debug.log("Saved to localStorage: cerebras,", model);
    }
  };
      ...
      <Select value={getCurrentModelValue()} onValueChange={handleModelChange}>
        ...
        <SelectContent>
          ...
          <SelectGroup>
            ...
            <SelectItem value="cerebras-llama3.1-8b">
              <div className="flex items-center gap-3">
                <Bot className="h-4 w-4 flex-shrink-0" />
                <div className="flex flex-col min-w-0">
                  <span className="truncate text-sm">Cerebras Llama 3.1 8B</span>
                  <span className="text-xs text-muted-foreground truncate">
            ...
            <SelectItem value="cerebras-llama3.1-70b">
              <div className="flex items-center gap-3">
                <Bot className="h-4 w-4 flex-shrink-0" />
                <div className="flex flex-col min-w-0">
                  <span className="truncate text-sm">Cerebras Llama 3.1 70B</span>
                  <span className="text-xs text-muted-foreground truncate">
                    Ultra-fast large model
                  </span>
                </div>
              </div>
            </SelectItem>
