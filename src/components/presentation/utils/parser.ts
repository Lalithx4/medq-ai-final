...
export class SlideParser {
  ...
  public parseChunk(chunk: string): PlateSlide[] {
    ...
    if (this.buffer.length === 0 && chunk.length > 0) {
      debug.log("🔍 [PARSER] First chunk received, length:", chunk.length);
    }
  ...
  public finalize(): PlateSlide[] {
    debug.log("🏁 [PARSER] Finalize - Total slides:", this.completedSections.length);
    // Finalizing parser
    ...
    try {
      ...
      // Skip PRESENTATION tag if present (case-insensitive)
      const upperBuffer = remainingBuffer.toUpperCase();
      if (upperBuffer.startsWith("<PRESENTATION")) {
  ...
  private processSections(): PlateSlide[] {
    ...
    debug.log("⚙️ Processing", this.completedSections.length, "completed sections");
    const newSlides = this.completedSections.map(this.convertSectionToPlate);
    debug.log("✨ Created", newSlides.length, "new slides");
  ...
  private extractCompleteSections(): void {
    debug.log("🔍 Extracting complete sections from buffer, length:", this.buffer.length);
    let startIdx = 0;
    let extractedSectionEndIdx = 0;

    // Handle potential PRESENTATION wrapper tag - skip it (case-insensitive)
    const upperBuffer = this.buffer.toUpperCase();
    const presentationStartIdx = upperBuffer.indexOf("<PRESENTATION");
    ...
    while (true) {
      ...
      if (
        ...
        debug.log("✅ Found complete section, total completed:", this.completedSections.length);
  ...
  private convertSectionToPlate = (sectionString: string): PlateSlide => {
    ...
    debug.log("🎨 Created slide:", {
      id: slideId,
      contentElements: plateElements.length,
      hasRootImage: !!rootImage,
      layoutType,
      rootImageQuery: rootImage?.query?.substring(0, 50)
    });
  ...
  private processTopLevelNode(node: XMLNode): PlateNode | null {
    ...
    switch (tag) {
      ...
      default:
        debug.warn(`Unknown top-level tag: ${tag}`);
        return null;
    }
  }
