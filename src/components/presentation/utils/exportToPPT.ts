...
export class PlateJSToPPTXConverter {
  ...
  private async addRootImage(rootImage: RootImage, layoutType?: string) {
    ...
    try {
      ...
      console.error("Failed to add root image:", {
        error,
        url: imagePath,
        layoutType,
        message: error instanceof Error ? error.message : "Unknown error"
      });
      // Re-throw to let caller know image failed
      throw new Error(`Failed to add image: ${imagePath}. ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async addImage(
    ...
    if (!measureOnly && imageUrl && this.currentSlide) {
      try {
        ...
        debug.warn("Failed to add image:", error);
      }
    }
  ...
  private async addSVGToSlide(
    ...
    try {
      ...
      debug.warn("Failed to add SVG:", error);
