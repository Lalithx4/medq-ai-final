...
export function ExportButton({
  ...
  const rootImageGeneration = usePresentationState((s) => s.rootImageGeneration);

  const handleExport = async () => {
    try {
      // Check if any images are still generating
      const hasGeneratingImages = slides.some((slide) => {
        if (slide.rootImage?.query && !slide.rootImage?.url) {
          const gen = rootImageGeneration[slide.id];
          return !gen || gen.status === "pending";
        }
        return false;
      });
      ...
      if (hasGeneratingImages) {
        toast({
          ...
          description: "Please wait for all images to finish loading before exporting.",
          variant: "destructive",
        });
        return;
      }
