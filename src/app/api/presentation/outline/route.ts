...
export async function POST(req: Request) {
  try {
    ...
    console.log("Starting outline generation with model:", modelProvider, modelId);
    ...
    return NextResponse.json(
      { error: "Failed to generate outline", details: error instanceof Error ? error.message : String(error) },
      { status: 500 },
    );
  }
}
