...
interface ShareMeetingDialogProps {
  ...
  onStartMeeting?: (stream: StreamingRoom) => void;
  onReschedule?: (stream: StreamingRoom) => void;
}

export function ShareMeetingDialog({ open, onOpenChange, stream, onStartMeeting, onReschedule }: ShareMeetingDialogProps) {
  ...
  // Check if meeting can start (15 min before scheduled time)
  const canStartMeeting = () => {
    if (isLive) return true;
    if (!scheduledDate) return true;
    const now = new Date();
    const startWindow = new Date(scheduledDate.getTime() - 15 * 60 * 1000); // 15 min before
    return now >= startWindow;
  };

  const handleStartMeeting = () => {
    if (onStartMeeting && stream) {
      onStartMeeting(stream);
      onOpenChange(false);
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-muted/50 rounded-lg space-y-3">
            <div className="flex items-start justify-between gap-2">
              ...
              {onReschedule && scheduledDate && stream.status !== 'live' && stream.status !== 'ended' && (
                <Button
                  variant="ghost"
                  size="sm"
          ...
          {onStartMeeting && (
            <>
              ...
              >
                ...
                {isLive ? 'Join Meeting' : canStartMeeting() ? 'Start Meeting' : 'Not Yet Available'}
              </Button>
              {!canStartMeeting() && scheduledDate && (
